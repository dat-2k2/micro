...
GPIOG_MODER = (GPIOG_MODER & ~(1ul<<15)) | (1ul<<14); //Set PG7 as GPO
GPIOG_MODER = (GPIOG_MODER &~(1ul<<13)) | (1ul<<12); //Set PG6 as GPO
GPIOG_MODER = (GPIOG_MODER & ~(1ul<<17)) | (1ul<<16); //Set PG8 as GPO
GPIOH_MODER = (GPIOH_MODER & ~(1ul<<5)) | (1ul<<4); //Set PH2 as GPO
GPIOH_MODER = (GPIOH_MODER & ~(1ul<<7)) | (1ul<<6); //Set PH3 as GPO
GPIOH_MODER = (GPIOH_MODER & ~(1ul<<13)) | (1ul<<12); //Set PH6 as GPO
GPIOH_MODER = (GPIOH_MODER & ~(1ul<<15)) | (1ul<<14); //Set PH7 as GPO
GPIOI_MODER = (GPIOI_MODER & ~(1ul<<21)) | (1ul<<20); //Set PI10 as GPO
for(i=0; i<4; i++){}
while(1)
 {
//PH3
